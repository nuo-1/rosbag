...
namespace gpu {
    ...
    bool MatrixHost::moveToGpu(MatrixDevice output) {
        ...
        if (offset_ == output.offset()) {
            checkCudaErrors(cudaMemcpy(output.buffer(), buffer_, sizeof(double) * rows_ * cols_ * offset_,
    ...
    bool MatrixHost::moveToHost(MatrixDevice input) {
        ...
        if (offset_ == input.offset()) {
            checkCudaErrors(cudaMemcpy(buffer_, input.buffer(), sizeof(double) * rows_ * cols_ * offset_,
