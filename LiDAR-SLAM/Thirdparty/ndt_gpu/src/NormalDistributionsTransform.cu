...
namespace gpu {
    ...
    double
    GNormalDistributionsTransform::auxilaryFunction_PsiMT(double a, double f_a, double f_0, double g_0, double mu) {
    ...
    void GNormalDistributionsTransform::computeTransformation(const Eigen::Matrix<float, 4, 4> &guess) {
        ...
        p << init_translation(0), init_translation(1), init_translation(2), init_rotation(0), init_rotation(
                1), init_rotation(2);
        ...
        while (!converged_) {
            ...
            delta_p_norm = computeStepLengthMT(p, delta_p, delta_p_norm, step_size_, transformation_epsilon_ / 2, score,
                                               score_gradient, hessian, trans_x_, trans_y_, trans_z_, points_number_);
                ...
                for (int j = starting_voxel_id[i]; j < starting_voxel_id[i + 1]; j++) {
                    ...
                    if (!(tmp_ex > 1 || tmp_ex < 0 || tmp_ex != tmp_ex)) {
                        ...
                        tmp_sg += ((d_x - centroid_x[vid]) * cov_dxd_pi_mat0[j] +
                                   ...
                                   (d_z - centroid_z[vid]) * cov_dxd_pi_mat2[j]) * tmp_ex;
                    }
                }
            ...
            for (int i = id; i < valid_points_num; i += stride) {
                ...
                for (int j = starting_voxel_id[i]; j < starting_voxel_id[i + 1]; j++) {
                    ...
                    if (!(tmp_ex > 1 || tmp_ex < 0 || tmp_ex != tmp_ex)) {
                        ...
                        final_hessian += -gauss_d2 *
                                         ((d_x - centroid_x[vid]) * cov_dxd0 + (d_y - centroid_y[vid]) * cov_dxd1 +
                                          (d_z - centroid_z[vid]) * cov_dxd2) * tmp_h[j] * tmp_ex;
            ...
            for (int i = id; i < valid_points_num; i += stride) {
                ...
                for (int j = starting_voxel_id[i]; j < starting_voxel_id[i + 1]; j++) {
                    ...
                    if (!(tmp_ex > 1 || tmp_ex < 0 || tmp_ex != tmp_ex)) {
                        ...
                        final_hessian +=
                                (d_x - centroid_x[vid]) * (icov00[vid] * ph0 + icov01[vid] * ph1 + icov02[vid] * ph2) *
                                tmp_ex;
                        final_hessian +=
                                (d_y - centroid_y[vid]) * (icov10[vid] * ph0 + icov11[vid] * ph1 + icov12[vid] * ph2) *
                                tmp_ex;
                        final_hessian +=
                                (d_z - centroid_z[vid]) * (icov20[vid] * ph0 + icov21[vid] * ph1 + icov22[vid] * ph2) *
                                tmp_ex;
...
/* Compute sum of score_inc list */
    ...
    double GNormalDistributionsTransform::computeDerivatives(Eigen::Matrix<double, 6, 1> &score_gradient,
                                                             Eigen::Matrix<double, 6, 6> &hessian,
        ...
        voxel_grid_.radiusSearch(trans_x, trans_y, trans_z, points_num, resolution_, INT_MAX, &valid_points,
                                 &starting_voxel_id, &voxel_id, &valid_voxel_num, &valid_points_num);
        ...
        if (compute_hessian) {
            computePointHessian0<<<grid_x, block_x>>>(x_, y_, z_, points_number_,
                                                      ...
                                                      point_hessians + valid_points_num * 63,
                                                      point_hessians + valid_points_num * 69,
                                                      point_hessians + valid_points_num * 75,
                                                      point_hessians + valid_points_num * 58,
                                                      point_hessians + valid_points_num * 81,
                                                      point_hessians + valid_points_num * 64,
                                                      point_hessians + valid_points_num * 87,
                                                      point_hessians + valid_points_num * 70,
                                                      point_hessians + valid_points_num * 93,
                                                      point_hessians + valid_points_num * 59,
                                                      point_hessians + valid_points_num * 99,
                                                      point_hessians + valid_points_num * 65,
                                                      point_hessians + valid_points_num * 105,
            ...
            computePointHessian1<<<grid_x, block_x>>>(x_, y_, z_, points_number_,
                                                      valid_points, valid_points_num,
                                                      dh_ang_.buffer(),
                                                      point_hessians + valid_points_num * 76,
                                                      point_hessians + valid_points_num * 82,
                                                      point_hessians + valid_points_num * 88,
                                                      point_hessians + valid_points_num * 94,
                                                      point_hessians + valid_points_num * 77,
                                                      point_hessians + valid_points_num * 100,
                                                      point_hessians + valid_points_num * 83,
                                                      point_hessians + valid_points_num * 106,
            ...
            computePointHessian2<<<grid_x, block_x>>>(x_, y_, z_, points_number_,
                                                      valid_points, valid_points_num,
                                                      dh_ang_.buffer(),
                                                      point_hessians + valid_points_num * 95,
                                                      point_hessians + valid_points_num * 101,
        ...
        computeExCovX<<<grid_x, block_x>>>(trans_x, trans_y, trans_z, valid_points,
                                           ...
                                           gauss_d1_, gauss_d2_,
                                           e_x_cov_x,
                                           inverse_covariance, inverse_covariance + voxel_num,
                                           inverse_covariance + 2 * voxel_num,
                                           inverse_covariance + 3 * voxel_num, inverse_covariance + 4 * voxel_num,
                                           inverse_covariance + 5 * voxel_num,
                                           inverse_covariance + 6 * voxel_num, inverse_covariance + 7 * voxel_num,
                                           inverse_covariance + 8 * voxel_num);
        checkCudaErrors(cudaGetLastError());

        computeScoreList<<<grid_x, block_x>>>(starting_voxel_id, voxel_id, valid_points_num, e_x_cov_x, gauss_d1_,
                                              score);
        checkCudaErrors(cudaGetLastError());
        ...
        if (compute_hessian) {
            ...
            computeHessianListS0<<<grid, block_x>>>(trans_x, trans_y, trans_z, valid_points,
                                                    ...
                                                    inverse_covariance, inverse_covariance + voxel_num,
                                                    inverse_covariance + 2 * voxel_num,
                                                    inverse_covariance + 3 * voxel_num,
                                                    inverse_covariance + 4 * voxel_num,
                                                    inverse_covariance + 5 * voxel_num,
                                                    inverse_covariance + 6 * voxel_num,
                                                    inverse_covariance + 7 * voxel_num,
                                                    inverse_covariance + 8 * voxel_num,
                                                    point_gradients,
    ...
    double GNormalDistributionsTransform::computeStepLengthMT(const Eigen::Matrix<double, 6, 1> &x,
...
//Copied from ndt.hpp
    ...
    void
    GNormalDistributionsTransform::computeHessian(Eigen::Matrix<double, 6, 6> &hessian, float *trans_x, float *trans_y,
                                                  float *trans_z, int points_num, Eigen::Matrix<double, 6, 1> &p) {
        int *valid_points, *voxel_id, *starting_voxel_id;
        int valid_voxel_num, valid_points_num;
        //Radius Search
        voxel_grid_.radiusSearch(trans_x, trans_y, trans_z, points_num, resolution_, INT_MAX, &valid_points,
    ...
    double GNormalDistributionsTransform::getFitnessScore(double max_range) {
        ...
        voxel_grid_.nearestNeighborSearch(trans_x, trans_y, trans_z, points_number_, valid_distance, min_distance,
                                          max_range);
